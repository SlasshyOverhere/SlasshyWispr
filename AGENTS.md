...
- **IPC:** `src/ipc/commands.ts` owns the command-name strings; `src/ipc/client.ts` owns typed invocation. Call sites use these, never raw `invoke()` with a string literal. Neither the names nor the payload fields are maintained twice: `src/ipc/wire-contract.test.ts` parses the Rust sources and fails on drift in either direction (a field Rust sends that TypeScript doesn't declare, or a command the frontend calls that `generate_handler!` doesn't list).
