...
- Window is a fixed **780x600** (non-resizable, non-maximizable, non-minimizable, custom titlebar with `decorations: false`), so this is an instrument footprint, not a floor. Both Tauri configs, `BASE_WINDOW_*` in `src/main.tsx` and `MAIN_WINDOW_MIN_SIZE` in `src/windows/dock-geometry.ts` must agree. Because nothing can widen the window, only the `max-width: 760px` (rail collapses to icons) and `max-width: 640px` (rail hides) breakpoints can ever fire — the older 1040px threshold collapsed the rail at 780 and is why it is 760.
