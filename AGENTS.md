...
- **Coqui is the only Python left.** `coqui_bridge.py` is *not* in the repo and is not embedded; `pipeline/tts/coqui.rs` and `pipeline/daemon/coqui.rs` resolve their script at runtime, and `services/coqui_setup.rs` builds its venv with pip. Zero-Python mode disables it — and that mode is **on by default**: `SLASSHYWISPR_ZERO_PYTHON_MODE` is an opt-*out* (`zero_python_mode_enabled()` defaults to true), so a stock build runs no Python at all.
