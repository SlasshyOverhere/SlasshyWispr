...
- Window is a fixed **780x600** (non-resizable, non-maximizable, non-minimizable, custom titlebar with `decorations: false`), so this is an instrument footprint, not a floor. Both Tauri configs, `BASE_WINDOW_*` in `src/main.tsx` and `MAIN_WINDOW_MIN_SIZE` in `src/windows/dock-geometry.ts` must agree. Because nothing can widen the window, only the `max-width: 760px` (page padding) and `max-width: 640px` (metric grid and list rows) breakpoints can ever fire. The shell is a top tab strip with no rail: the tabs are icons only, permanently — the label and hotkey live in the `title` tooltip, so there is no expanded state and no toggle.
