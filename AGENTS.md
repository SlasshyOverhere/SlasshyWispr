...
- **sherpa-onnx must be linked in shared mode**, not static: its Windows static archive is built `/MT` while Rust MSVC and our `ort` objects are `/MD`, so the static form fails with LNK2038/LNK1169. Shared linking puts four DLLs in the exe's import table; `scripts/stage-sherpa-runtime.mjs` (wired as `bundle.beforeBundleCommand`) copies them into `src-tauri/sherpa-runtime/`, which `bundle.resources` maps onto the install root so the Windows loader finds them beside the exe.
