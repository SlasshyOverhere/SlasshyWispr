...
const HOTKEY_SHIFTED_ALIASES: Record<string, string> = {
  "!": "1",
  "@": "2",
  "#": "3",
  $: "4",
...
const HOTKEY_PUNCTUATION = new Set([",", ".", "/", "\\", ";", "'", "`", "-", "=", "[", "]"]);
...
function normalizeHotkeyKeyToken(token: string): string {
  ...
  if (normalized.length === 1) {
    ...
    if (HOTKEY_SHIFTED_ALIASES[normalized]) {
      return HOTKEY_SHIFTED_ALIASES[normalized];
    }
    if (HOTKEY_PUNCTUATION.has(normalized)) {
      return normalized;
    }
  }
  ...
  return HOTKEY_KEY_MAP[normalized] ?? "";
}
