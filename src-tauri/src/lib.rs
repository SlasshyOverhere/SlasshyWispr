...
pub mod updater;
use commands::{
    StartupLocalSttWarmupTarget, TtsSetupState, capture_selected_text, check_for_app_update,
    ...
    show_update_settings, start_tts_runtime_setup, validate_coqui, validate_piper,
...
async fn run_assistant_pipeline(
    ...
    if should_try_backend_selection_capture {
        #[cfg(target_os = "windows")]
        {
            match commands::input::capture_selected_text_windows() {
                Ok(captured) => {
...
fn copy_last_transcript_to_clipboard(app: &AppHandle) {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Err(error) = commands::input::set_clipboard_text_windows(&transcript) {
            error!(
...
fn copy_last_response_to_clipboard(app: &AppHandle) {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Err(error) = commands::input::set_clipboard_text_windows(&response) {
            error!(
