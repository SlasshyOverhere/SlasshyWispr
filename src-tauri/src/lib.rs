...
fn load_startup_local_stt_warmup_target(app: &AppHandle) -> StartupLocalSttWarmupTarget {
    let settings_path = match services::settings_store::persisted_settings_path(app) {
        Ok(path) => path,
    ...
    match model {
        Some(model) => StartupLocalSttWarmupTarget::Model(crate::pipeline::routing::canonical_local_stt_model_id(model)),
...
fn read_launch_at_login_preference(app: &AppHandle) -> bool {
    let path = match services::settings_store::persisted_settings_path(app) {
        Ok(p) => p,
        Err(_) => return true,
    };
    if !path.exists() {
