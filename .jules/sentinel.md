...
## 2026-03-30 - [HIGH] Missing Input Validation on Command Binaries
**Vulnerability:** `Command::new()` was invoked with variables (e.g. `python_path`, `piper_path`, `bootstrap_python`) that were not validated directly prior to execution. This left a gap where modified logic could inadvertently pass an arbitrary string into `Command::new()`, causing arbitrary execution.
**Learning:** Even if a helper like `resolve_coqui_python_path` validates the input, the direct consumers of that variable should enforce validation right before spawning to guarantee defense-in-depth.
**Prevention:** Added `validate_python_binary_path` and `validate_piper_binary_path` directly to the beginning of functions like `setup_coqui_runtime_blocking`, `setup_local_stt_runtime_blocking`, `run_local_stt_python_command`, and `spawn_coqui_bridge_daemon`.

## 2026-03-30 - [HIGH] Command Injection in Windows Registry Configuration via PowerShell
**Vulnerability:** The `configure_launch_at_login` Tauri command updated the Windows Registry using dynamically generated strings passed to `run_powershell_script`. Although partially mitigated by `[string]::IsNullOrWhiteSpace` and `.Replace`, spawning a PowerShell subprocess with concatenated strings (e.g., executable paths) poses a command injection risk.
**Learning:** Shelling out to `powershell.exe` to perform basic OS operations is a common source of command injection vulnerabilities, has poor performance characteristics, and often triggers false positives in Endpoint Detection and Response (EDR) solutions.
**Prevention:** Instead of using shell scripts or intermediate binaries, always use direct, native API calls. For Windows Registry modifications in Rust, utilize FFI bindings to `advapi32.dll` (e.g., `RegCreateKeyExW`, `RegSetValueExW`) via `windows-sys` or custom FFI declarations to ensure type safety and eliminate shell injection risks.
